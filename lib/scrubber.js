var ScrubbingSpeed = (function() {
  ...
  function checkClose(){
    ...
    if(Psypher.close){
      document.getElementById('ephemeral').parentNode.remove();
      (!Psypher.owner) && Psypher.set_owner();
      Psypher.new_session && Psypher.owner.add_session(Psypher.new_session);
    }
  }
  /******************************************************************
  Cache
  ...
  function Scrubber(name, callback, speedsOverride) {
    ...
    ******************************************************************/
    ...
    function updateHelpers(e) {
      ...
      isPlaceAbove = true; // isPlaceAbove = y > window.innerHeight * .5;
    }
