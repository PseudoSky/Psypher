...
**[See the demo](http://padolsey.github.com/bend/demo.html)**

`bend` searches for regular expression matches in a given DOM node and replaces or wraps each match with a node or piece of text that you can specify.
...
```js
bend(document.getElementById('t'), {
  find: /Hello/,
  wrap: 'em'
});
```
...
Grab the latest [bend.js](https://github.com/padolsey/bend/blob/master/src/bend.js) (or `npm install bend`) and include it as a `<script>` on your page.

## API / Usage

`bend` has the following argument signature:
 ...
 * **filterElements** *optional* (`Function`): A function to be called on every element encountered by `bend`. If the function returns false the element will be altogether ignored.
...
The most common usage of `bend` is to replace text found in regular prose, not all DOM nodes. To make this easier there is a preset that you can use to instruct it to:
...
A portion or "match portion" is a part of a match that is delimited by node boundaries. Not all matches occur within a single text-node, so `bend` has to be able to deal with cross-boundary matches (e.g. when matching `/foo/` in `"<em>f</em>oo"`).
...
```js
bend(document.getElementById('container'), {
  find: 'function',
...
Calling `bend` returns an instance of an internal Finder constructor -- the API on the object is limited, at the moment, to reverting:
...
```js
bend(document.getElementById('test'), {
  find: 'amazing',
  wrap: 'em'
});
```
 ...
 * 0.4.4 (4 May 2015): Remove duplicate key from `NON_CONTIGUOUS_PROSE_ELEMENTS`. Expose library via UMD ([See #32](https://github.com/padolsey/bend/issues/32)).
 * 0.4.3 (28 Apr 2015): Add `preset:prose` and `forceContext` features. [See #29](https://github.com/padolsey/bend/issues/29).
 * 0.4.2 (30 Mar 2014): Fix IE to avoid incorrectly-closed-tags issue ([see #20](https://github.com/padolsey/bend/issues/20)). Thanks to [shauryamal](https://github.com/shauryamal)!
 * 0.4.1 (11 Mar 2014): Fix portionMode:first phantom nodes (see [issue #19](https://github.com/padolsey/bend/issues/19))
 * 0.4.0 (6 Oct 2013): Major API overhaul, including a new arg signature (`bend(node, options)`, plus the ability to replace a match with text or wrap it with a DOM Node.
 * 0.3.0: Switch to semver, add node-filtering feature (as requested in [Issue #11](https://github.com/padolsey/bend/issues/11)
 * 0.2: Fix case where regular expression contains word bounderies and add support for specifying a capture group to replace as the fourth argument to `bend()` (see [issue #5](https://github.com/padolsey/bend/issues/5))
 * 0.11: Minor fix: Make sure replacement node function is called in order of matches (see [issue #4](https://github.com/padolsey/bend/issues/4))
 * 0.1: Initial commit + Fix for IE's broken HTML5 cloneNode ([pull request](https://github.com/padolsey/bend/pull/3))
